...
def ground(collector) -> dict:
    ...
    # 4) Freshness / SLA / source health / duplicates
    if latest.get("latency_ms", 0) > config.SLA_LATENCY_MS:
        risk += 10.0
        evidence.append("latency over SLA")
    ...
    lead_ticks = ttf_ticks if ttf_ticks not in (None, 0) else None

    # confidence from trend fit + amount of accumulated history
    hist = len(collector.history)
    confidence = round(min(0.95, 0.4 + 0.4 * null_r2 + min(0.15, hist / 100.0)), 2)
